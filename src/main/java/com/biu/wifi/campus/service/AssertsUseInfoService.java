...
public class AssertsUseInfoService extends AbstractAuditUserService {
    ...
    public void audit(AssertsUseInfo info, Short status, String remark) {
        ...
        if (status.intValue() == 3) {
            ...
            // 审核驳回,通知请假人请假结果
            assertsUseNotice =  assertsUseNoticeService.addUseNotice(info.getId(), "您的请假申请未通过审批", info.getReason(), remark, info.getApplyUserId());
            ...
            if (currentAuditUserIsTheLast) {
                ...
                assertsUseInfoMapper.updateByPrimaryKeySelective(info);
                // 通知请假人请假结果
                assertsUseNotice = assertsUseNoticeService.addUseNotice(info.getId(), "您的请假申请已通过审批", info.getReason(), remark, info.getApplyUserId());
                Assert.notNull(assertsUseNotice.getId(), "审核失败");
                // 推送请假审批结果通知给请假人
                receiverId = info.getApplyUserId();
                title = "您提交的请假已通过审批";
