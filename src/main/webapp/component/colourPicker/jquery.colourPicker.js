jQuery.fn.colourPicker = function (conf) {
    ...
    return this.each(function () {
        ...
        //var input    = jQuery('<input type="text" name="' + select.attr('name') + '" value="' + select.val() + '" size="3" />').insertAfter(select);
        var input = jQuery('<input type="text" name="' + select.attr('name') + '" size="1" readonly="readonly" />').insertAfter(select);

        //初始化颜色
        input.css({background: config.initColor, color: '#' + hexInvert(config.initColor)});
        ...
        icon.click(function () {
            ...
            jQuery('a', colourPicker).click(function () {
                ...
                //调用回调
                if (config.v_callFunction != "" && window[config.v_callFunction]) {
                    window[config.v_callFunction].call(window, "#" + hex);
                }
                //input.val(hex);
