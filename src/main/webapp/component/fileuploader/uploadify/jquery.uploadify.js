    ...
    var aq = "undefined", aD = "object", ab = "Shockwave Flash", X = "ShockwaveFlash.ShockwaveFlash",
        ...
        aU = false, aH, an, aI = true, ah = function () {
            var a = typeof aL.getElementById != aq && typeof aL.getElementsByTagName != aq && typeof aL.createElement != aq,
                e = aB.userAgent
                    .toLowerCase(), c = aB.platform.toLowerCase(), h = c ? /win/.test(c) : /win/.test(e),
                j = c ? /mac/.test(c) : /mac/.test(e), g = /webkit/
                    .test(e) ? parseFloat(e.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : false, d = !+"\v1", f = [0, 0, 0],
                k = null;
    ...
    function ae(f, d, h, e) {
        ...
        if (a) {
            ...
            var b = ah.ie && ah.win ? "ActiveX" : "PlugIn",
                c = "MMredirectURL=" + af.location.toString().replace(/&/g, "%26")
...
SWFUpload.prototype.getFlashHTML = function () {
    return ['<object id="', this.movieName, '" type="application/x-shockwave-flash" data="', this.settings.flash_url, '" width="',
        this.settings.button_width, '" height="', this.settings.button_height, '" class="swfupload">', '<param name="wmode" value="',
        this.settings.button_window_mode, '" />', '<param name="movie" value="', this.settings.flash_url, '" />',
        ...
        onSelect: function (h) {
            ...
            for (var g in this.queueData.files) {
                ...
                if (f.uploaded != true && f.name == h.name) {
                    //TODO 不需要提示
                    var e = true;// confirm('文件名称为 "' + h.name + '" 的文件已经在上传队列.\n是否需要重新上传?');
                    if (!e) {
            ...
            if (c.inArray("onSelectError", e.overrideEvents) < 0) {
                switch (g) {
                    ...
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT :
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 超过了限制大小 (' + e.fileSizeLimit + ").";
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE :
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 是空文件.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT :
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 不是允许的类型 (' + e.fileTypeDesc + ").";
                        break;
                }
            }
        ...
        onUploadError: function (e, i, h) {
            ...
            switch (i) {
                ...
                case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED :
                    //alert("The upload limit has been reached (" + h + ")."); //TODO 不需要报错
        ...
        onUploadStart: function (d) {
            ...
            if (e.checkExisting) {
                c.ajax({
                    ...
                    success: function (h) {
                        if (h == 1) {
                            var g = confirm('A file with the name "' + d.name
