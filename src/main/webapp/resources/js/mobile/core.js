...
document.writeln("<scri" + "pt src='resources/js/mobile/js/fastclick.min.js' type='text/javascript'></sc" + "ript>");
...
AE.mobile = function (serviceName, params, v_callback, options) {
...
/** ********************* MobileJSBridge ******************************* */
// 如果是ios系统
if (navigator.userAgent.toLowerCase().indexOf("xcframeios") > -1) {
    window.JSBridge = {};
    window.JSBridge.IOSMobileJSBridge = {};
    document.writeln("<scri" + "pt src='resources/js/mobile/bridge/WebViewJavascriptBridge.js?v=2' type='text/javascript'></sc" + "ript>");
    document.addEventListener('WebViewJavascriptBridgeReady', _init_ios_bridge, false);
}

function _init_ios_bridge() {

    var _bridge = window.WebViewJavascriptBridge;

    if (_bridge && !_bridge._messageHandler) {
...
MobileJSBridge.call = function (v_methodName, v_param, v_callback, v_options) {
    ...
    if (window.JSBridge && window.JSBridge.AndroidMobileJSBridge) {
    ...
    } else if (window.JSBridge && window.JSBridge.IOSMobileJSBridge) {
        ...
        if (_bridge) {
            ...
            // 进行调用
            try {
                _bridge.callHandler(v_methodName, v_param, v_callback);
            } catch (e) {
            }
        }
    }
...
AE.UI = {
    ...
    showwaitform: function () {
        ...
        //
        // // 请求处理计数器
        // window.top["frame_req_counter"] = window.top["frame_req_counter"] ?
        // window.top["frame_req_counter"] + 1 : 1;
        //
        // window.top.$("#ajax-loading").show();
        //
